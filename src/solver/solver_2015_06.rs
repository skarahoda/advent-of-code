...
fn get_instructions() -> Vec<Instruction> {
    ...
    let input: &str = &input[..];
    re.captures_iter(input).map(|captures: Captures| {
        Instruction {
            start: ((&captures)[2].parse().unwrap(), (&captures)[3].parse().unwrap()),
            end: ((&captures)[4].parse().unwrap(), (&captures)[5].parse().unwrap()),
...
fn solve_second_part(instructions: &Vec<Instruction>) -> usize {
        ...
        for x in instruction.start.0..=instruction.end.0 {
            for y in instruction.start.1..=instruction.end.1 {
                match instruction.command {
                    ...
                    Command::Off => {
                        lights[x][y] = lights[x][y].checked_sub(1).unwrap_or(0)
                    },
    ...
    lights.iter().fold(
        0,
        |acc: usize, row: &[usize;1000]| {
            row.iter().fold(
                acc,
                |acc: usize, light: &usize| acc + light
            )
        }
    )
}
