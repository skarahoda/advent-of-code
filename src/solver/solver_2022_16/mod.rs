...
fn solve_first_part(valves: &Valves) -> i32 {
    ...
    let shortest_path_map: HashMap<&str, HashMap<String, i32>> = valves
        .keys()
    ...
    while let Some((score, remaining_minutes, current_point, visited_points)) = candidates.pop() {
        ...
        for (next_point, distance) in shortest_paths {
            ...
            if !visited_points.contains(next_point) && remaining_minutes_after_open > 0 {
                let score_for_valve =
                    remaining_minutes_after_open * valves.get(next_point).unwrap().flow;
...
fn solve_second_part(valves: &Valves) -> i32 {
    ...
    let shortest_path_map: HashMap<&str, HashMap<String, i32>> = valves
        ...
        .map(|point| (point.as_str(), get_shortest_paths(valves, point)))
        .collect();
    let mut sorted_flows: Vec<(&str, i32)> = valves
        .iter()
...
pub fn solve() -> (i32, i32) {
    ...
    (solve_first_part(&valves), solve_second_part(&valves))
}
...
mod tests {
        ...
        assert_eq!(
            ...
            HashMap::from([
                (
                    ...
                    Valve::new(
                        0,
                        HashSet::from(["DD".to_string(), "II".to_string(), "BB".to_string(), ])
                    )
                ),
                (
                    "BB".to_string(),
                    Valve::new(13, HashSet::from(["CC".to_string(), "AA".to_string()]))
                ),
                (
                    "CC".to_string(),
                    Valve::new(2, HashSet::from(["DD".to_string(), "BB".to_string()]))
                ),
                (
                    "DD".to_string(),
                    Valve::new(
                        20,
                        HashSet::from(["CC".to_string(), "AA".to_string(), "EE".to_string()])
                    )
                ),
                (
                    "EE".to_string(),
                    Valve::new(3, HashSet::from(["FF".to_string(), "DD".to_string()]))
                ),
                (
                    "FF".to_string(),
                    Valve::new(0, HashSet::from(["EE".to_string(), "GG".to_string()]))
                ),
                (
                    "GG".to_string(),
                    Valve::new(0, HashSet::from(["FF".to_string(), "HH".to_string()]))
                ),
                (
                    "HH".to_string(),
                    Valve::new(22, HashSet::from(["GG".to_string()]))
                ),
                (
                    "II".to_string(),
                    Valve::new(0, HashSet::from(["AA".to_string(), "JJ".to_string()]))
                ),
                (
                    "JJ".to_string(),
                    Valve::new(21, HashSet::from(["II".to_string()]))
                ),
            ])
        );
    }
    ...
    fn should_solve_first_part() {
        ...
        assert_eq!(solve_first_part(&valves), 1651);
    }

    #[test]
    fn should_solve_second_part() {
        let valves = get_valves(EXAMPLE);
        assert_eq!(solve_second_part(&valves), 1707);
    }
}
