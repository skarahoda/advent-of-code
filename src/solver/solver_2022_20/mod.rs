...
fn decrypt(numbers: &Vec<isize>, key: isize, rounds: usize) -> isize {
    let mut numbers: Vec<(usize, isize)> = numbers
        .iter()
        .enumerate()
        .map(|(i, &number)| (i, number * key))
        .collect();
    for _ in 0..rounds {
...
pub fn solve() -> (isize, isize) {
    ...
    (solve_first_part(&numbers), solve_second_part(&numbers))
}
...
mod tests {
    ...
    fn should_get_numbers() {
        assert_eq!(get_numbers(EXAMPLE), vec![1, 2, -3, 3, -2, 0, 4]);
    }

    #[test]
    fn should_solve_first_part() {
        assert_eq!(solve_first_part(&get_numbers(EXAMPLE)), 3);
    }

    #[test]
    fn should_solve_second_part() {
        assert_eq!(solve_second_part(&get_numbers(EXAMPLE)), 1623178306);
    }
}
