    ...
    fn evaluate(&self, old: &u64) -> u64 {
        match self {
            Operation::Add(left, right) => {
                left.evaluate(old) + right.evaluate(old)
            },
...
impl Monkey {
    ...
    fn throw<F>(&self, item: &u64, relief: F) -> (usize, u64) where F: Fn(u64) -> u64 {
        let item = relief(self.operation.evaluate(item));
        let monkey_index = if item % self.tester == 0 { self.if_true } else { self.if_false };
        (monkey_index, item)
    }
}
...
fn get_monkeys(input: &str) -> HashMap<usize, Monkey> {
    ...
    for monkey_pair in monkey_pairs {
        ...
        let items: Vec<u64> = pairs[1].to_owned().into_inner()
...
fn solve_part<F>(monkeys: &mut HashMap<usize, Monkey>, rounds: usize, relief: &F) -> u64 where F: Fn(u64) -> u64 {
            ...
            for item in monkey.items.clone() {
                ...
                *investigation_map.entry(current_index).or_insert(0) += 1;
                ...
                while current_index <= next_index {
                    ...
                    *investigation_map.entry(current_index).or_insert(0) += 1;
                    (next_index, item) = monkeys.get(&current_index).unwrap().throw(&item, relief).clone();
                }
                new_items_map.entry(next_index).or_insert(Vec::new()).push(item);
            }
        }
...
fn solve_second_part(input: &str) -> u64 {
    let mut monkeys = get_monkeys(&input);
    let relief = monkeys.values().fold(1, |product, monkey| product * monkey.tester);
